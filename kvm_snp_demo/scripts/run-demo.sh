...
run_demo() {
    if [[ "$1" == "snp" ]]; then
        ./cloud-hypervisor --cpus boot=4 --memory size=8000000K --platform sev_snp=true \
            ...
            --disk path=disks/noble-server-cloudimg-amd64.raw path=disks/ubuntu-cloudinit.img  \
            ...
            --fw-cfg-config initramfs=off --serial file=chv-serial.log --log-file ch-fw.log -vvv;
    else 
